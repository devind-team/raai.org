<template lang="pug">
div(v-if="!profilesValueLoading && !profilesLoading")
      ...
      v-data-table(
        ...
        template(#item.value="{ item }")
          template(v-if="pvs[item.code]")
            ...
            v-tooltip(v-if="canChange" bottom)
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const { mutate: ChangeProfileValueMutate } = useMutation<
      ChangeProfileValueMutation,
      ChangeProfileValueMutationVariables
    >(
      changeProfileValueMutation,
      {
        update: (cache, result) => update(
          cache,
          result,
          (dataCache, { data: { changeProfileValue: { profileValue } } }
          ) => {
            const dataKey: string = Object.keys(dataCache)[0]
            dataCache[dataKey] = dataCache[dataKey].filter(
              (e: ProfileValueType) => e.profile.id !== profileValue.profile.id
            )
    ...
    const changeProfileValue = (profile: ProfileType, newValue: string): void => {
      ChangeProfileValueMutate({ userId: props.user.id, profileId: profile.id, value: newValue })
    }

    const { mutate: ChangeProfileVisibilityMutate } = useMutation<
      ChangeProfileVisibilityMutation,
      ChangeProfileVisibilityMutationVariables
    >(
      changeProfileVisibilityMutation,
    ...
    const pvs = computed<ProfileAggregation>(() => (
